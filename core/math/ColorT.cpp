...
bool Color32::operator==(const Color32& rhs) const { return a == rhs.a && r == rhs.r && g == rhs.g && b == rhs.b; }

bool Color32::operator!=(const Color32& rhs) const { return !(rhs == *this); }
...
Color32 Color32::RandomColor(int min, int max) { return Color32(Random::Range(min, max), Random::Range(min, max), Random::Range(min, max), 255); }

Color32 Color32::RandomColor(int rmin, int rmax, int gmin, int gmax, int bmin, int bmax, int amin, int amax) { return Color32(0); }
...
void Colorf::RGBToHSVHelper(float offset, float dominantcolor, float colorone, float colortwo, float* H, float* S, float* V) {
  *V = dominantcolor;
  if ((double)*V != 0.0) {
